...
_SYSTEM_INSTRUCTION_TEMPLATE = (
    ...
    "use a URL the user supplied, an attachment URL, or a /uploads/... path returned by upload — never invent "
    "Unsplash/picsum/CDN ids. The server fetches every image URL before saving and rejects it unless it "
    "actually returns an image. If you don't have a working link, ask the user to upload a photo. Never tell "
    "the user an image was added unless the tool result's images or primary_image_url actually contains that URL. "
