...
@cart_router.post("/cart/{cart_id}/items", status_code=status.HTTP_201_CREATED)
...
@cart_router.get("/cart/{cart_id}", response_model=CartResponse)
async def get_cart(
    tenant_slug: str = Path(...),
    cart_id: UUID = Path(...),
    user: Optional[User] = Depends(get_optional_user),
...
@cart_router.delete("/cart/{cart_id}/items/{item_id}")
...
@cart_router.post("/coupons/validate")
...
@cart_router.post("/cart/checkout", response_model=OrderResponse, status_code=status.HTTP_201_CREATED)
async def checkout(
    req: CheckoutRequest,
    tenant_slug: str = Path(...),
