...
async def seed():
    # This script runs on the platform plane (creating tenants themselves,
    # not acting as one) -- same as super_admin_service.py and auth_service's
    # signup path, every insert below needs unscoped() since there's no
    # bound tenant, and each TenantScoped row already sets its own
    # tenant_id explicitly (required inside unscoped(), see
    # app/db/session.py's _stamp_and_guard_writes).
    with unscoped():
