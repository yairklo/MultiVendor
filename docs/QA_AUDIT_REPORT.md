...
- ~~**Operational note:** the backend pytest suite's `auto_clear_db` autouse fixture truncates and reseeds the whole database... stomp on each other's fixtures.~~ **Fixed:** `pytest` and the dev server/E2E suite now use fully separate MySQL databases (`multivendor_test` vs `multivendor_dev`). `tests/conftest.py` sets `DATABASE_URL`/`DB_NAME` env vars to `multivendor_test` before `app.main` is imported (so `app/db/session.py`'s `get_db()` — used by every route the tests hit — targets the test DB too, not just conftest's own fixtures), and `db/seed.sql` now runs `USE multivendor_test`. `server/app/core/config.py`, the root `.env`, `docker-compose.yml`, and `db/schema.sql` all default to `multivendor_dev`. Running the full pytest suite no longer touches dev data — verified by snapshotting `multivendor_dev`'s tenant/user counts before and after a full run.
